...
class PanelStatusTable extends Component {
  ...
  static forkOnGithub() {
    window.location.assign('https://github.com/lucky0604')
  }
    ...
    const rows = this.props.panels.map((panel) => {
      return (
        <Table.Row key={panel.id}>
          ...
          <Table.Cell className="panel-status-table--data-cell">
            <a title={'View Panel ' + panel.id + ' details'}>{panel.id}</a>
          </Table.Cell>
          <Table.Cell disabled={!panel.enabled} className="panel-status-table--data-cell">{panel.inputRadianceKWM2.toFixed(2)} kW/㎡</Table.Cell>
          <Table.Cell disabled={!panel.enabled} className="panel-status-table--data-cell">{panel.outputVoltageV.toFixed(2)} V</Table.Cell>
          <Table.Cell disabled={!panel.enabled} className="panel-status-table--data-cell">{panel.outputCurrentA} A</Table.Cell>
        </Table.Row>
      )
    })
    return (
        ...
        <Table unstackable celled compact definition className='panel-status-table--table'>
          <Table.Header fullWidth>
            <Table.Row>
              ...
              <Table.HeaderCell>Output VoltageV</Table.HeaderCell>
              <Table.HeaderCell>Output CurrentA</Table.HeaderCell>
            </Table.Row>
          </Table.Header>
