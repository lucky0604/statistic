...
class PowerOutputChart extends Component {
  constructor(props) {
      ...
      tooltips: {
        callbacks: {
          ...
          title: (tooltipItem, data) => {
            ...
            return data.dataset[datasetIndex].label
          },
    ...
    // store these values in the component state so React re-renders the component whenever these values changed
    this.state = {
   ...
   * @param {panels} panels An array containing `panel` objects
   */
  static getTotalOutputPower(panels) {
    return panels.reduce((accumulator, panel) => {
      const outputPowerN = panel.outputVoltageV * panel.outputCurrentA
      const outputPowerKW = outputPowerN / 1000
      return accumulator + outputPowerKW
    }, 0)
  }

  render() {
    // Construct the `data` object in the format the `Line` component exists
    const data = {
