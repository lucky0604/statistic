...
class UserController @Inject()(users: Users)(cc: ControllerComponents) extends AbstractController(cc) {
  ...
  */
  ...
  def login = Action.async { implicit request =>
    ...
    userOpt.map {
      case Some(u) =>
        u.password == Some(passwordBase64ForSHA) match {
          case true =>
  ...
  def reg = Action.async {implicit request =>
    ...
    val password = Base64.encodeBase64String(shaEncoder.digest(regFormTuple._2.getBytes()))
    val user = User(Option(regFormTuple._1), Option(password), Option(regFormTuple._3))
    // val captchaText = request.session.get("captcha")
    if (regFormTuple._1 != "" && Option(password) != "") {
      {
        for {
