...
class Users @Inject()(articles: Articles)(protected val dbConfigProvider: DatabaseConfigProvider)
extends HasDatabaseConfigProvider[JdbcProfile] {
  ...
  private class UsersTable(tag: Tag) extends Table[User](tag, "user") {
    ...
    def utype = column[Option[Int]]("utype")
    ...
    def * = (name, password, email, phone, descrp, utype, status, qopenid, qtoken, sopenid, stoken, inittime, updtime, tombstone, uid) <> ((User.apply _).tupled, User.unapply)
  }
  ...
  def _queryByEmail(uemail: String): DBIO[Option[User]] = table.filter(_.email === uemail).result.headOption

  def _queryByPhone(uphone: String): DBIO[Option[User]] = table.filter(_.phone === uphone).result.headOption
  ...
  def queryByName(uname: String): Future[Option[User]] = {
    db.run(_queryByName(uname))
  }

  def queryByEmail(uemail: String): Future[Option[User]] = {
    db.run(_queryByEmail(uemail))
  }

  def queryByPhone(uphone: String): Future[Option[User]] = {
    db.run(_queryByPhone(uphone))
  }
  ...
  def userIsExists(uname: String): Future[Boolean] = {
    val ret = for {
      ...
      uemailExists <- _queryByEmail(uname)
      uphoneExists <- _queryByPhone(uname)
    } yield {
      unameExists.isDefined || uemailExists.isDefined || uphoneExists.isDefined
    }
