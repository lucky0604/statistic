...
case class ArticleListWrapper(article: List[Article], count: Int)
...
class Articles @Inject()(protected val dbConfigProvider: DatabaseConfigProvider) extends HasDatabaseConfigProvider[JdbcProfile] {
  ...
  class ArticlesTable(tag: Tag) extends Table[Article](tag, "article") {
    ...
    def atype = column[Option[Int]]("atype")
  ...
  def updateReadCount(aid: Long, read: Int): Future[Int] = {
    db.run(
      table.filter(_.aid === aid).filter(_.tombstone === 0)
        .map(row => (row.read, row.updtime)).update(Some(read), Option(System.currentTimeMillis() / 1000L))
    )
  }

  def updateSmileCount(aid: Long, smile: Int): Future[Int] = {
    db.run(
      table.filter(_.aid === aid).filter(_.tombstone === 0).map(
        row => (row.smile, row.updtime)
      ).update(Some(smile), Option(System.currentTimeMillis() / 1000L))
    )
  }
  ...
  def revoke(aid: Long): Future[Int] = {
    db.run(
      table.filter(_.aid === aid).filter(_.tombstone === 0).map(
        row => (row.tombstone, row.updtime)
      ).update(Some(1), Option(System.currentTimeMillis() / 1000L))
    )
  }
}
